...
**국문 뉴스 병행 수집 (`네이버뉴스크롤링.ipynb`)**

같은 전시회 키워드를 네이버 검색 API로도 돌려 국문 기사를 수집했습니다.
`display=100`으로 페이지네이션하며 최대 1,000건까지 가져오고, 제목+요약 텍스트에서
무기 키워드를 부분일치로 추출한 뒤 영문 토큰은 위와 같은 방식으로 표제어 복원합니다.
아래 **3. Market Analysis**에 실린 국가별 막대그래프가 이 경로의 산출물입니다.
...
이 표는 감이 아니라 **뉴스 기사에서 추출한 무기 키워드 빈도**에서 나왔습니다.
각 국가의 방산 전시회명으로 뉴스를 수집해, 기사 제목·요약에 등장한 무기 키워드를 집계한 결과입니다.

### 동유럽 — 전차 / 자주포

![폴란드 무기 키워드 빈도](assets/news-keywords/poland_weapon_keywords.png)

폴란드는 `fa-50`이 압도적으로 많고, 그 뒤로 `tank`, `전차 파워팩`, `howitzer`가 이어집니다.
지상장비 중심 수요 → **K2 전차 / K9 자주포**를 앞세운 현대로템 매칭 근거.

### 중동 — 드론 / 미사일

![사우디 무기 키워드 빈도](assets/news-keywords/saudi_weapon_keywords.png)

사우디는 `drone`(33)과 `missile`(32)이 3위 `weapon`(19)과 큰 차이로 앞섭니다.
정밀타격·무인체계 수요 → **LIG넥스원** 매칭 근거.

### 남미 — 항공기

![칠레 무기 키워드 빈도](assets/news-keywords/chile_weapon_keywords.png)

칠레는 `aircraft`(18)가 1위이고 `t-50`, `jet`, `fa-50`이 함께 잡힙니다.
훈련기·경공격기 수요 → **한국항공우주(KAI)** 매칭 근거.

> 전체 12개국(미국·영국·독일·폴란드·루마니아·사우디·UAE·이라크·태국·브라질·칠레·콜롬비아)의
> 막대그래프와 워드클라우드는 분석 노트북 저장소의 `네이버뉴스크롤링_그래프/`에 있습니다.
